...
func (cfg *config) nCommitted(index int) (int, logEntry) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		// println("nCommitted index: " + strconv.Itoa(index))
		// println("nCommitted nextIndex for server " + strconv.Itoa(i) + ": " + strconv.Itoa(nextIndex))
		cfg.mu.Unlock()
	...
	for time.Since(t0).Seconds() < 10 {
		...
		for si := 0; si < cfg.n; si++ {
			...
			if rf != nil {
				...
				if ok {
					index = index1
					//fmt.Println("leader " + strconv.Itoa(starts) + " accepted command in index " + strconv.Itoa(index1))
					break
				}
			}
		}
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				if nd > 0 && nd >= expectedServers {
					// committed
					// println("----------------- Config: Committed! ------------------")
					if e == (logEntry{true, cmd}) {
						// println("----------------- CONFIRMED: Committed! ------------------")
			...
			if retry == false {
				log.Fatalf("one(%v) failed to reach a agreement", cmd)
			}
		} else {
